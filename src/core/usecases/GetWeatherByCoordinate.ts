...
export class GetWeatherByCoordinate
  implements UseCase<GetWeatherByCoordinateInput, Promise<Weather>>
{
  constructor(
  ...
  async execute(input: GetWeatherByCoordinateInput): Promise<Weather> {
    ...
    const getWeather = await this.weatherGateway.getWeatherByCoordinate(
      lon,
      lat
    );
